...
class datos_modelo
{
  ...
  }function editar_casa($propietario,$apartamento,$edificio,$id)
  {
    ...
    function crear_casa($propietario,$edificio,$apartamento)
    {
      ...
      public function obtener_cuentas_pendientes_de($apartamento,$edificio)
      {
        $mysql=new conexion();
        $sql="SELECT * FROM `cuentas`  WHERE `cuentas`.`apartamento`=".$apartamento." AND `cuentas`.`edificio`=".$edificio." ORDER BY  `cuentas`.`pendiente` DESC;";
        $con=$mysql->conectar();
        $resultado= mysqli_query($con,$sql);
      ...
      public function obtener_casas()
      {
       ...
       public function obtener_casas_de($id)
        {
          $mysql=new conexion();
          $sql="SELECT * FROM `casas` WHERE `casas`.`propietario` =".$id." ;";
          $con=$mysql->conectar();
          $resultado= mysqli_query($con,$sql);
